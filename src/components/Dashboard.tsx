...
import { SettingsView, UserProfile } from './SettingsView';
    ...
    <div className="bg-white rounded-2xl p-5 sm:p-6 shadow-sm border border-gray-200">
      ...
      <div className="flex items-start justify-between mb-0">
        <div className="flex items-start gap-3">
           ...
           <h2 className="text-lg font-bold text-gray-900 leading-tight tracking-tight pt-0.5">
...
const ActiveMissionSection = ({ 
    ...
    return (
      <div className="bg-white rounded-2xl p-6 sm:p-8 border border-dashed border-gray-300 text-center flex flex-col items-center justify-center shadow-sm">
        ...
        <button 
          ...
          className="bg-[#2F5BFF] hover:bg-blue-600 text-white text-xs font-bold py-3 px-6 rounded-xl shadow-lg shadow-blue-500/20 transition-all active:scale-95 flex items-center gap-2"
        >
  ...
  return (
    <div className={`relative transition-all duration-500 ${isMissionActive ? '' : ''}`}>
      <div 
        className={`bg-white rounded-2xl p-5 sm:p-6 relative overflow-hidden group transition-all duration-500 ${
          isMissionActive 
            ? 'shadow-[0_20px_40px_-12px_rgba(47,91,255,0.25)] border-2 border-[#2F5BFF] ring-4 ring-[#2F5BFF]/5' 
      ...
      >
        <div className="flex justify-between items-center mb-4">
          <div className="flex items-center gap-2">
            <div className={`w-2 h-2 rounded-full ${isMissionActive ? 'bg-orange-500 animate-pulse' : 'bg-gray-300'}`}></div>
        ...
        <div className="flex gap-3">
          <button 
            onClick={onToggle}
            className={`flex-1 text-white font-bold py-4 rounded-xl shadow-lg active:scale-[0.98] transition-all flex items-center justify-center gap-2 text-sm tracking-wide ${
              isMissionActive 
          ...
          {isMissionActive && (
            <button 
              ...
              className="w-16 bg-emerald-500 hover:bg-emerald-600 text-white rounded-xl shadow-lg shadow-emerald-500/20 active:scale-[0.98] transition-all flex items-center justify-center"
        ...
        <div className="space-y-1">
          {nextMissions.map((mission) => (
            <button 
              ...
              className="w-full flex items-center justify-between group cursor-pointer hover:bg-gray-50 p-3 -mx-3 rounded-xl transition-colors active:scale-[0.99] text-left"
            >
...
const DailyCloseSection = ({ isBlurred, missions, weeklyFocus }: { isBlurred?: boolean; missions: Mission[]; weeklyFocus: string }) => {
  ...
  React.useEffect(() => {
    ...
    const storedTimestamp = localStorage.getItem('dailySummaryTimestamp');
    if (storedSummary && storedTimestamp) {
      // Check if summary is from today
      if (new Date(storedTimestamp).toDateString() === new Date().toDateString()) {
  ...
  const handleGenerateReport = () => {
    ...
    if (win) {
      report += `Your biggest win was: "${win}". `;
    }

    if (friction) {
      report += `You encountered some friction with: "${friction}". `;
    }
    ...
    if (weeklyFocus) {
      report += `This aligned with your weekly focus on "${weeklyFocus}".`;
    }
    ...
    localStorage.setItem('dailySummary', report);
    localStorage.setItem('dailySummaryTimestamp', timestamp);

    // 4. Trigger Webhook
    const webhookUrl = localStorage.getItem('vector_webhook_report');
    if (!webhookUrl) {
      alert("Add webhook in Settings.");
    } else {
      fetch(webhookUrl, {
        method: 'POST',
  ...
  return (
    <div className={`bg-white rounded-2xl shadow-sm border border-gray-200 transition-all duration-500 overflow-hidden ${isBlurred ? 'opacity-50 grayscale-[0.5]' : ''}`}>
      <button 
        ...
        className="w-full flex items-center justify-between p-5 sm:p-6 hover:bg-gray-50/50 transition-colors"
      >
          ...
          >
            <div className="p-5 sm:p-6 space-y-6">
              ...
              <button 
                ...
                className={`w-full mt-4 font-bold py-4 rounded-xl active:scale-[0.98] transition-all flex items-center justify-center gap-2 text-white ${
                  showSummary 
                    ? 'bg-[#5A75D6] hover:bg-[#4A65C6] shadow-md shadow-blue-500/10' 
                    : 'bg-[#3B64FF] hover:bg-[#2F5BFF] shadow-lg shadow-blue-500/20'
                }`}
              >
                <RotateCcw size={18} className="text-white/90" />
                <span>{showSummary ? 'REGENERATE REPORT' : 'GENERATE REPORT'}</span>
              </button>
...
const BottomNav = ({ activeTab, setActiveTab }: { activeTab: string, setActiveTab: (tab: string) => void }) => {
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 bg-white/90 backdrop-blur-xl border-t border-gray-200 pb-8 pt-2 px-6 z-50">
...
export default function Dashboard() {
  ...
  const [weeklyFocus, setWeeklyFocus] = useState(() => {
    return localStorage.getItem('vector_weekly_focus') || "Polish UI & Fix Critical Bugs";
  });
  ...
  // Load user from localStorage on mount (Legacy / Offline fallback)
  React.useEffect(() => {
  ...
  // Save weeklyFocus to localStorage
  React.useEffect(() => {
    localStorage.setItem('vector_weekly_focus', weeklyFocus);
  }, [weeklyFocus]);
  ...
  const createMission = async (missionData: {
    ...
    console.debug("createMission called", missionData);
    ...
    try {
      ...
      console.debug("Mission created successfully");
    } catch (error) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-[#F2F2F7] to-[#E5E5EA] text-gray-900 font-sans pb-10 selection:bg-[#2F5BFF]/20">
      {/* Header */}
      <header className="sticky top-0 z-40 bg-[#F2F2F7]/90 backdrop-blur-md px-4 sm:px-6 py-3 flex justify-between items-center">
      ...
      <main className="px-4 sm:px-5 max-w-md mx-auto pt-2 pb-24">
        {activeTab === 'Today' && (
          <div className="space-y-6 pt-6">
            <GreetingHeader user={user} onAvatarClick={handleAvatarClick} streak={streak} completedToday={completedToday} />
            ...
            />
            <DailyCloseSection isBlurred={isMissionActive} missions={missions} weeklyFocus={weeklyFocus} />
          </div>
        )}
